...
//TODO: Welche PIN Nummer hat die LED -> schaue dazu auf dem Sensor-Kit und ersetze unten PIN mit der Nummer (ohne das D)
int LED = PIN; //Hier speichern wir eine Zahl (der PIN von unserer LED) auf das Wort LED (LED ist die Variabel)
...
void loop() { //Der Loop-Bereich wird immer wiederholt solange das Arduino läuft
  ...
  int licht = analogRead(A?); // lese die Werte aus dem Lichtsensor und speichere diese auf der Variable (Wort) Licht ab.
